...
void addAllFilesToMap(string path, map<string, string>& filePaths, vector<string>& fileNames){
    ...
    for (auto const& recursiveFile : fs::recursive_directory_iterator(path)) { 
        ...
        if (is_directory(recursiveFile)) { 
            for (auto const&  file : fs::directory_iterator{ recursiveFile }) { 
                count++; 
            } 
        } 
        ...
        if (count > 1) {
            addAllFilesToMap(recursiveFile.path().string(), filePaths, fileNames);
        }

        //Add files to map and vector
        if (!is_directory(recursiveFile)) {
            filePaths[recursiveFile.path().string()] = recursiveFile.path().string();
            fileNames.push_back(recursiveFile.path().string());
        }
    }
    return;
}